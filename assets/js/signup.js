...
(async () => {
    // Handle Supabase error hashes (e.g. expired OTP) or query params (e.g. Google OAuth errors)
    const hash = window.location.hash;
    const search = window.location.search;
    
    let errDesc = null;
    if (hash && hash.includes('error=')) {
        errDesc = new URLSearchParams(hash.substring(1)).get('error_description');
    } else if (search && search.includes('error=')) {
        errDesc = new URLSearchParams(search).get('error_description');
    }
